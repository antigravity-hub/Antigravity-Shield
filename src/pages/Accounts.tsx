...
function Accounts() {
  ...
  const executeRefresh = async () => {
    ...
    try {
      ...
      if (!isBatch) {
        // High-speed parallel batch refresh across all accounts via backend
        setRefreshingIds(new Set(accounts.map((a) => a.id)));
        const stats = await refreshAllQuotas();
        if (stats.failed === 0) {
          showToast(
            t("accounts.refresh_selected", { count: stats.success }),
            "success",
          );
        } else {
          showToast(
            `${t("common.success")}: ${stats.success}, ${t("common.error")}: ${stats.failed}`,
            "warning",
          );
        }
        return;
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <button
              ...
              className={cn(
                "px-2.5 py-1.5 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 border cursor-pointer active:scale-95 shadow-xs select-none",
                isAutoWarmEnabled
                  ? "bg-orange-500/10 hover:bg-orange-500/20 text-orange-600 dark:text-orange-400 border-orange-500/30"
                  : "bg-gray-100 dark:bg-base-200 hover:bg-gray-200 dark:hover:bg-base-300 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-base-300"
              )}
              title={t('settings.warmup.title', 'Auto-Warm (100% Quota Recovery Warmup)')}
            >
              ...
              <span className={cn(
                ...
                isAutoWarmEnabled ? "bg-orange-500 text-white" : "bg-gray-300 dark:bg-gray-700 text-gray-600 dark:text-gray-300"
              )}>
                {isAutoWarmEnabled ? "ON" : "OFF"}
              </span>
              <span className={cn(
                "w-2 h-2 rounded-full",
                isAutoWarmEnabled ? "bg-orange-500 animate-pulse shadow-[0_0_6px_rgba(249,115,22,0.8)]" : "bg-gray-400"
              )} />
            </button>
